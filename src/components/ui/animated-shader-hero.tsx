...
void main(){gl_Position=position;}`;
  ...
  render(now = 0) {
    ...
    if (this.uPointers) gl.uniform2fv(this.uPointers, this.pointerCoords as unknown as Float32List);
...
const useShaderBackground = () => {
  ...
  useEffect(() => {
    ...
    const raf = (now: number) => {
      ...
      rendererRef.current.updateMove(pointersRef.current?.move ?? [0, 0]);
